...
function reorderDownloadSections() {
  var os = getOS();
  if (os) {
    var sections = document.getElementsByClassName("download-section");
    for (var section of sections) {
      if (section.classList.contains(os) && (section != sections[0])) {
        section.parentElement.insertBefore(section, sections[0]);
      }
    }
  }
}
